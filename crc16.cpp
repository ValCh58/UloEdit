...
//---------------------------------------------------------------------------
/// rасчитанная таблица значений полинома дл¤ просчета 16-ти битной контрольной суммы
...
//---------------------------------------------------------------------------
unsigned short CRC16(unsigned short *buf, unsigned short len)
{
    register int i;
    ...
    cp = buf;
    for(i=0; i<len; i++){
        crc = crc16tab[((crc >> 8) & 255)] ^ (crc << 8) ^ *cp;
        cp++;
    }
    return crc;
}
